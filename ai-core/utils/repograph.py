...
class RepoGraph:
    ...
    def _build_graph(self):
        ...
        logger.info(
            f"RepoGraph: Indexed {len(self.definitions)} definitions across codebase."
        )
    ...
    def _parse_file(self, full_path: str, rel_path: str):
        ...
        if "pages" in rel_path or "routes" in rel_path or "app" in rel_path:
            route_slug = (
                "/"
                if base_name in ("index", "page", "home", "landing")
                else f"/{base_name}"
            )
        ...
        patterns = [
            ...
            (
                r"(?:export\s+)?const\s+([A-Z][a-zA-Z0-9_]*)\s*=\s*(?:React\.)?(?:memo|forwardRef)?\s*\(",
                "component",
            ),
            # React hooks: function useSomething, const useSomething =
            (
                r"(?:export\s+)?(?:default\s+)?function\s+(use[A-Z][a-zA-Z0-9_]*)",
                "hook",
            ),
    ...
    def localize_fault(
        self, error_message: str, url: str = "", stack_trace: Optional[str] = None
        ...
        stack_match = re.search(
            r"([a-zA-Z0-9_\-\./]+\.(?:jsx?|tsx?|py)):(\d+)", combined_text
        )
        if stack_match:
            ...
            for node in self.definitions.values():
                if node.file_path.endswith(matched_file) or matched_file.endswith(
                    os.path.basename(node.file_path)
                ):
                    return {
...
if __name__ == "__main__":
    ...
    result = graph.localize_fault(
        "TypeError: Cannot read properties of undefined in Login component",
