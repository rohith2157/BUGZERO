            ...
            <AnimatePresence mode="wait">
                ...
                >
                    {text.split(" ").map((word, wordIndex) => (
                        <span key={`word-${count}-${wordIndex}`} className="inline-flex whitespace-nowrap">
                            {word.split("").map((char, charLocalIndex) => {
                                const priorChars = text.split(" ").slice(0, wordIndex).join(" ").length;
