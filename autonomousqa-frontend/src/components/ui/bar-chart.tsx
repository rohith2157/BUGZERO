...
export interface BarChartContextValue {
    data: Record<string, unknown>[]; xScale: ScaleBandType<string>; yScale: ScaleLinearType<number>;
    width: number; height: number; innerWidth: number; innerHeight: number; margin: Margin;
    bandWidth: number; tooltipData: TooltipData | null; setTooltipData: Dispatch<SetStateAction<TooltipData | null>>;
    containerRef: RefObject<HTMLDivElement | null>; bars: BarConfig[]; isLoaded: boolean; animationDuration: number;
    xDataKey: string; hoveredBarIndex: number | null; setHoveredBarIndex: (index: number | null) => void;
    orientation: "vertical" | "horizontal"; stacked: boolean; stackGap: number;
    stackOffsets: Map<number, Map<string, number>>; barGap: number; barWidth?: number;
}

interface BarConfig { dataKey: string; fill: string; stroke?: string; }
...
function TooltipIndicator({ x, height, visible, width = 1, colorEdge = chartCssVars.crosshair, colorMid = chartCssVars.crosshair, fadeEdges = true, gradientId = "bar-tooltip-indicator-gradient" }: {
    ...
    useEffect(() => { animatedX.set(x - width / 2); }, [x, animatedX, width]);
    if (!visible) return null;
...
function TooltipBox({ x, y, visible, containerRef, containerWidth, containerHeight, offset = 16, className = "", children, top: topOverride }: {
    ...
    offset?: number; className?: string; children: ReactNode; top?: number | ReturnType<typeof useSpring>;
}) {
    ...
    return createPortal(
        <motion.div animate={{ opacity: 1 }} className={cn("pointer-events-none absolute z-50", className)} exit={{ opacity: 0 }} initial={{ opacity: 0 }} ref={tooltipRef} style={{ left: animatedLeft, top: finalTop }} transition={{ duration: 0.1 }}>
            <motion.div animate={{ scale: 1, opacity: 1, x: 0 }} className="min-w-[140px] overflow-hidden rounded-lg bg-popover text-popover-foreground shadow-lg backdrop-blur-md" initial={{ scale: 0.85, opacity: 0, x: shouldFlipX ? 20 : -20 }} key={flipKey} style={{ transformOrigin }} transition={{ type: "spring", stiffness: 300, damping: 25 }}>
                {children}
            </motion.div>
        </motion.div>,
        container
...
export function ChartTooltip({ showCrosshair = true, showDots = true, content, rows: rowsRenderer, children, className = "" }: ChartTooltipProps) {
    ...
    const title = useMemo(() => { if (!tooltipData) return undefined; return String(tooltipData.point[xDataKey] ?? ""); }, [tooltipData, xDataKey]);
        ...
        <>
            {showCrosshair && !isHorizontal && (
                <svg aria-hidden="true" className="pointer-events-none absolute inset-0" height="100%" width="100%">
                    <g transform={`translate(${margin.left},${margin.top})`}><TooltipIndicator height={innerHeight} visible={visible} width={1} x={x} /></g>
                </svg>
            )}
            {showDots && visible && !isHorizontal && (
                <svg aria-hidden="true" className="pointer-events-none absolute inset-0" height="100%" width="100%">
                    <g transform={`translate(${margin.left},${margin.top})`}>
                        {bars.map((bar) => <TooltipDot color={bar.stroke || bar.fill} key={bar.dataKey} strokeColor={chartCssVars.background} visible={visible} x={tooltipData?.xPositions?.[bar.dataKey] ?? x} y={tooltipData?.yPositions[bar.dataKey] ?? 0} />)}
                    </g>
                </svg>
            )}
...
export interface BarXAxisProps { tickerHalfWidth?: number; showAllLabels?: boolean; maxLabels?: number; }
...
export function BarXAxis({ tickerHalfWidth = 50, showAllLabels = false, maxLabels = 12 }: BarXAxisProps) {
    ...
    const labelsToShow = useMemo(() => {
        ...
        if (!showAllLabels && labels.length > maxLabels) { const step = Math.ceil(labels.length / maxLabels); labels = labels.filter((_, i) => i % step === 0); }
        return labels;
    ...
    return createPortal(
        <div className="pointer-events-none absolute inset-0">
            {labelsToShow.map((item) => {
                let opacity = 1;
                if (isHovering && crosshairX !== null) { const fadeBuffer = 20; const fadeRadius = tickerHalfWidth + fadeBuffer; const distance = Math.abs(item.x - crosshairX); if (distance < tickerHalfWidth) opacity = 0; else if (distance < fadeRadius) opacity = (distance - tickerHalfWidth) / fadeBuffer; }
                return (<div className="absolute" key={item.label} style={{ left: item.x, bottom: 12, width: 0, display: "flex", justifyContent: "center" }}><motion.span animate={{ opacity }} className="whitespace-nowrap text-chart-label text-xs" initial={{ opacity: 1 }} transition={{ duration: 0.4, ease: "easeInOut" }}>{item.label}</motion.span></div>);
            })}
        </div>,
        container
    );
...
export interface BarYAxisProps { showAllLabels?: boolean; maxLabels?: number; }
...
export function BarYAxis({ showAllLabels = true, maxLabels = 20 }: BarYAxisProps) {
    ...
    const labels = useMemo(() => {
        const domain = xScale.domain();
        let items = domain.map((label) => ({ label, y: (xScale(label) ?? 0) + bandWidth / 2 + margin.top }));
        if (!showAllLabels && items.length > maxLabels) { const step = Math.ceil(items.length / maxLabels); items = items.filter((_, i) => i % step === 0); }
        return items;
...
function extractBarConfigs(children: ReactNode): BarConfig[] {
    ...
    Children.forEach(children, (child) => {
        ...
        if (isBarComponent && props?.dataKey) configs.push({ dataKey: props.dataKey, fill: props.fill || "var(--chart-line-primary)", stroke: props.stroke });
    });
    return configs;
}
...
function BarChartInner({ width, height, data, xDataKey, margin, animationDuration, barGap, barWidth, orientation, stacked, stackGap, children, containerRef, yMax }: {
    ...
    const handleMouseMove = useCallback((event: React.MouseEvent<SVGGElement>) => {
        ...
        const chartX = point.x - margin.left; const chartY = point.y - margin.top;
        const domain = xScale.domain(); let foundIndex = -1;
        for (let i = 0; i < domain.length; i++) { const cat = domain[i]!; const bandStart = xScale(cat) ?? 0; const bandEnd = bandStart + bandWidth; if (isHorizontal ? (chartY >= bandStart && chartY <= bandEnd) : (chartX >= bandStart && chartX <= bandEnd)) { foundIndex = i; break; } }
        if (foundIndex >= 0) {
