...
export const SecureMessageGateway = ({
    ...
    return (
        <div className="flex items-center justify-center min-h-[300px] w-full p-4 bg-transparent">
      ...
      `,
                ...
                <div className="backdrop-blur-xs p-2 rounded-xs">
                    ...
                    {/* INPUT & BUTTON FLEX */}
                    <form onSubmit={handleSubmit} className="flex sm:flex-row flex-col items-stretch gap-2 relative z-10">
                        ...
                        <button
                            ...
                            className={cn(
                                "px-8 h-10 sm:h-auto border bg-card font-bold uppercase text-[0.6rem] tracking-[0.2em] min-h-10 border-dashed transition-all flex items-center justify-center gap-2 rounded-none",
                                !isPending && message.trim() && "hover:border-green-700 hover:text-primary hover:bg-green-300/5 active:scale-95",
                                (isPending || !message.trim()) && "opacity-40 cursor-not-allowed"
                            )}
                        >
                            <Send size={12} className={cn(isPending && "animate-bounce")} />
                ...
                <div className="mt-6 flex items-center justify-between relative z-10">
                    <span className="text-[0.55rem] font-mono uppercase tracking-widest text-muted-foreground">
                        Status: {isPending ? <span className="text-muted-foreground">Establishing Handshake...</span> : <span className="text-green-500 border p-0.5 border-green-500 ">Ready</span>}
                    </span>
